...
*/
export var CrimeService = (function () {
    ...
    CrimeService.prototype.save = function (data) {
        var _this = this;
        ...
        return query.get(CrimeTypeObjcId).then(function (object) {
            ...
            var time = new Date(data['reportTime']);
            crime.set('date_time', new Date(_this.formatDateTime(date, time)));
            return crime.save();
        }).then(function (crime) {
    ...
    CrimeService.prototype.formatDateTime = function (date, time) {
        // 2016-08-17T02:30:00.000+3:00
        var timezone = -(new Date().getTimezoneOffset() / 60);
        var timezoneStr = (timezone > 0) ? "+" + this.addZero(timezone) + ":00" : "-" + this.addZero(timezone) + ":00";
        var date_time = date.getFullYear() + "-" + date.getMonth() + "-" + date.getDate() + "T" + this.addZero(time.getHours()) + ":" + this.addZero(time.getMinutes()) + ":00" + timezoneStr;
        return date_time;
    };
    CrimeService.prototype.addZero = function (i) {
        if (i < 10) {
            i = "0" + i;
        }
        return i;
    ...
    //
    //   });    //end promise
    CrimeService.decorators = [
        { type: Injectable },
    ];
    /** @nocollapse */
    CrimeService.ctorParameters = [
        { type: Http, },
    ];
    return CrimeService;
}());
